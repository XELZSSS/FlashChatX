// Enhanced DeepSeek proxy with retry, caching, and connection pooling
...
const PATH_TO_PROVIDER = Object.entries(API_PROVIDERS).reduce(
  (acc, [provider, config]) => {
...
const TIME_TOOL_PROVIDERS = new Set([
  'openai',
  'openai-compatible',
  'deepseek',
  'z',
...
const handleOpenAIFileUpload = async (req, res) => {
  try {
    const parsed = await parseJsonBody(req, MAX_UPLOAD_BYTES);
    const apiKey = parsed?.apiKey || apiKeys.openai;
    if (!apiKey) {
...
const handleGoogleFileUpload = async (req, res) => {
  try {
    const parsed = await parseJsonBody(req, MAX_UPLOAD_BYTES);
    const apiKey = parsed?.apiKey || apiKeys.google;
    if (!apiKey) {
      return sendJson(res, 401, {
...
const handleAnthropicFileUpload = async (req, res) => {
  try {
    const parsed = await parseJsonBody(req, MAX_UPLOAD_BYTES);
    const apiKey = parsed?.apiKey || apiKeys.anthropic;
    if (!apiKey) {
      return sendJson(res, 401, {
...
async function retryRequest(requestFn, maxRetries = 3, baseDelay = 1000) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.log(
        `[proxy] Request failed, retrying in ${Math.round(delay)}ms (attempt ${attempt + 1}/${maxRetries})`
      );
...
const getEnvValue = key => process.env[key] || envMap[key];
...
const persistEnvUpdates = updates => {
...
const handleStreamResponse = async (upstream, res, options = {}) => {
  if (!upstream.body) {
    return sendJson(res, 500, { error: 'Upstream response missing body' });
  }
  ...
  return connectionPool.execute(async () =>
    retryRequest(async () => {
      ...
      if (provider === 'openai-compatible') {
        const overrideUrl = apiUrl || getEnvValue(config.urlEnvVar);
        if (overrideUrl) {
...
const handleApiRequest = async (req, res, provider, parsed) => {
  const config = API_PROVIDERS[provider];
  const { messages = [], model, stream = false } = parsed;

  const requestApiKey = parsed?.apiKey;
  const requestApiUrl = parsed?.apiUrl;
  const apiKey = requestApiKey || apiKeys[provider];

  if (requestApiKey) {
    apiKeys[provider] = requestApiKey;
  }
  ...
  const canUseSystemTimeTool =
    TIME_TOOL_PROVIDERS.has(provider) &&
    provider !== 'google' &&
    provider !== 'anthropic';
  ...
  if (shouldUseSystemTimeTool) {
    try {
      ...
      const mergedTools = mergeTools(normalizedPayload.tools, SYSTEM_TIME_TOOL);
...
const handleSaveEnv = async (req, res) => {
  try {
    ...
    const validProviders = Object.keys(API_PROVIDERS);
    ...
    try {
      ...
      const updatedKeys = persistEnvUpdates(updates);
...
const handleSaveMemuEnv = async (req, res) => {
  try {
    const payload = await parseJsonBody(req);
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && pathName === '/api/search/aggregate') {
    try {
      ...
      const apiKey = payload?.apiKey || getEnvValue('SEARCH_API_KEY') || '';
  ...
  const provider = PATH_TO_PROVIDER[pathName];
  if (provider) {
    return handleApiRequest(req, res, provider, parsedBody);
  }
